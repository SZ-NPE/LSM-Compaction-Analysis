...
filePath = "file_to_parse.txt"  # output file path
...
for line in Lines:  # loop
    ...
    # 判断一行的行首
    # cmpt_sty 为 compaction style 代表系统参数配置
    # #p_ts_in_tree 代表最后执行完的统计信息
    # %space_amp 代表最后的几个放大系数的统计
    # 相应的值添加到了 values 数组
    ...
    # 获取第一个 word
    current_line_first_word = get_first_word(line)  # get first word of current line
    ...
    # 如果是 rocksdb. 开头的内部信息统计
    # remove characters and put all numbers in a list 移除字符，转换成数字
    ...
    # 如果是 user_key_comparison_count 开头的信息
    # 读取操作的一些统计信息
    if current_line_first_word == "user_key_comparison_count":
